...
- Use a smaller ``tensor_parallel_size``. 
  ...
  Our experient in Sec. 8.4 of Anonymous evaluate this trade-off.

More tuning details such as dealing with Preemption and Chunked-prefill
can be found in Anonymous 

For optimal performance, we recommend using vLLM v0.8.3 or later. See https://github.com/anonymous-org/verl/blob/main/docs/README_vllm0.8.md for details.
...
Users can add the desired model config to the  `test_transformer.py <https://github.com/anonymous-org/verl/blob/main/tests/models/test_transformer.py#L24>`_ file.
...
If the test passes, you can add your desired model into the model `registry.py <https://github.com/anonymous-org/verl/blob/main/verl/models/registry.py#L24>`_ file.
...
- **Algorithmic metrics** (train batch size, PPO mini-batch size) are *global* (from a single-controller perspective), 
  normalized in each worker. See the Anonymous.
...
1. **Increase** ``actor_rollout_ref.actor.ppo_max_token_len_per_gpu``  
   Make it at least 2 x (max_prompt_length + max_response_length). We set it to 3x in `run_qwen2-7b_rm_seq_balance.sh <https://github.com/anonymous-org/verl/blob/main/examples/ppo_trainer/run_qwen2-7b_rm_seq_balance.sh#L25>`_.
...
3. **Use larger limits for Critic and Reward models**:
   ...
   `run_qwen2-7b_rm_seq_balance.sh <https://github.com/anonymous-org/verl/blob/main/examples/ppo_trainer/run_qwen2-7b_rm_seq_balance.sh#L40>`_
