...
`main_ppo.py <https://github.com/anonymous-org/verl/blob/main/verl/trainer/main_ppo.py>`_.
In this tutorial, we will go through the code architecture in `main_ppo.py <https://github.com/anonymous-org/verl/blob/main/verl/trainer/main_ppo.py>`_.
...
files in Anonymous. Currently, we support
...
For example, we already provide reward functions for Anonymous 
and Anonymous
...
See Anonymous for detailed implementation.
...
pre-defined several roles in `ray_trainer.py <https://github.com/anonymous-org/verl/blob/main/verl/trainer/ppo/ray_trainer.py#L38>`_.
...
- If it's a model-based RM, directly add the ``RewardModel`` role in the
  ...
  - Note that the pre-defined ``RewardModelWorker`` only supports models
    ...
    need to define your own RewardModelWorker in Anonymous 
    and Anonymous.
...
See reward functions implemented in Anonymous 
for more information.
