...
- ``actor_rollout_ref.actor.kl_loss_type``: Support ``kl`` (``k1``), ``abs``, ``mse`` (``k2``), ``low_var_kl`` (``k3``) and ``full``. How to calculate the kl divergence between actor and reference policy. For specific options, refer to `kl_penalty()` in `core_algos.py <https://github.com/anonymous-org/verl/blob/main/verl/trainer/ppo/core_algos.py>`_ . See this blog post for detailed analysis: http://joschu.net/blog/kl-approx.html
...
- ``kl_penalty``: Support ``kl``, ``abs``, ``mse``, ``low_var_kl`` and ``full``. How to
  ...
  specific options, refer to `kl_penalty()` in `core_algos.py <https://github.com/anonymous-org/verl/blob/main/verl/trainer/ppo/core_algos.py>`_ .
...
- ``target_modules``: The names of the modules to apply the adapter to,
  default to ``all-linear``. See Anonymous for detail.
