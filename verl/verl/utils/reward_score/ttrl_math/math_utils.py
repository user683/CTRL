...
_LATEX_PARSER_NOISE_SUPPRESSED = False


def _suppress_latex_parser_noise() -> None:
    global _LATEX_PARSER_NOISE_SUPPRESSED
    if _LATEX_PARSER_NOISE_SUPPRESSED:
        return
    for logger_name in ("pylatexenc", "pylatexenc.latexwalker", "latex2sympy2_extended"):
        logging.getLogger(logger_name).setLevel(logging.ERROR)
    _LATEX_PARSER_NOISE_SUPPRESSED = True


def _consume_latex_atom(expr: str, start: int) -> tuple[str, int]:
    """Read one LaTeX atom: braced group, command, or plain token."""
    n = len(expr)
    i = start
...
def sanitize_latex_fraction(expr: str) -> str:
    """Normalize malformed frac forms: frac{a}{b}, \frac12, \frac1{2}, \frac{1}2."""
    if not expr:
        return expr

    expr = expr.replace("\\dfrac", "\\frac")
    expr = expr.replace("\\tfrac", "\\frac")
    expr = re.sub(r"(?<!\\)\\b(?:dfrac|tfrac|frac)\\b", r"\\frac", expr)
    ...
    while i < n:
        if expr.startswith("\\frac", i):
            j = i + len("\\frac")
            num, j = _consume_latex_atom(expr, j)
            den, j = _consume_latex_atom(expr, j)
            if num and den:
...
def is_latex_equal(given_answer: str, ground_truth: str) -> bool:
    try:
        with timeout(1):
            try:
                _suppress_latex_parser_noise()
                given_answer = sanitize_latex_fraction(given_answer)
                ground_truth = sanitize_latex_fraction(ground_truth)
...
def _parse_latex(expr: str) -> str:
    ...
    _suppress_latex_parser_noise()
    expr = sanitize_latex_fraction(expr)
    expr = expr.replace("\\tfrac", "\\frac")
    expr = expr.replace("\\dfrac", "\\frac")
